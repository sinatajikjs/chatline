...
import MuiCountrySelect from "../MaterialComponents/MuiCountrySelect";
...
const Signin = () => {
  ...
  const handleChange = ({ target }) => {
    ...
    setInputValue(value);
    if (value.length < 8) setDisabled(true);
  ...
  useEffect(() => {
    axios.get("http://ip-api.com/json").then((res) => {
      const targetCountry = countries.find((c) => {
        return c.code == res.data.countryCode;
      });
      setInputValue(targetCountry.phone);
    });
  }, []);
  ...
  return !codeSent ? (
    ...
    <VerifyCode inputValue={inputValue} setCodeSent={setCodeSent} />
  );
};
